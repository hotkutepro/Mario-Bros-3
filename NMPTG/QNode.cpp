...
void QNode::LoadObjects(string path)//Viet lai
{
	...
	fstream open(path);
	string line;
	//TYPEOBJECT typeO[21] = { brick, coin, drain, land, leaf, mushroom_red, p, box, question_block, star, tarnooki,
	//tarnooki_fly, tortoise, tortoise_fly, tortoise_red, tree, tree_red, tree_red_shoot, tree_shoot,uprise,music };
	for (int i = 0; i < 21; i++)
	{
		open >> stype;
		open >> count;
